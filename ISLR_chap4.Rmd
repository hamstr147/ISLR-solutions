---
title: "ISLR Chapter 4 solutions"
...
Next, we take logs (another monotonic transformation):
...
Again, because we are maximising for a fixed value of $x$ and all classes have a common variance $\sigma^2$, $\frac{x^2}{2\sigma^2}$ is a constant and so adding it to this expression is another monotonic transformation.
...
Producing some graphical summaries of the `Weekly` data set:
```{r}
lags <- Weekly %>% 
  select(-Volume, -Today) %>% 
...
Volumes have gone up considerably since 1990, with a notable levelling off in about 2002 to 2003, and a drop from 2008. returns. The box plots show that returns appear to fluctuate around zero; it appears that where stocks have gone down this week, returns were more frequently higher last week and five weeks ago, than stocks that went up. The opposite is true for two weeks, with higher returns for stocks that went up this week, and there is no discernable relationship for 3 and 4 week lags. There is no relationship between trading volumes and current returns. Returns are more volatile in the recent part of the data set, with significantly negative returns past the 900th observation. 

Fitting a logistic regression to predict `Direction`:
...
Computing the confusion matrix for this model:
...
The predictions appear to suggest that the logistic regression model predicts a high number of false positives - i.e. when the market goes down, logistic regression usually predicts up. 

Now create a training set from 1990 to 2008:
...
And calculate the accuracy and confusion matrix for the test data:
...
Fit an LDA model on the training and assess accuracy on the test set:
...
KNN with $k$ = 1 perfoms worst of all, it's no better than a coin flip. 
...
```
None of the KNN classifiers perform particularly well - choosing `Up` every would perform better than any of them. They are clearly outperformed by the single predictor LDA and logistic regression classifiers. 

## Exercise 11
**In this problem, you will develop a model to predict whether a given car gets high or low gas mileage based on the `Auto` data set.**

Create a binary variable that is 1 where `mpg` is above median and 0 where it is below:
```{r}
mpg01 <- ifelse(Auto$mpg > median(Auto$mpg), 1, 0)
auto_new <- data.frame(Auto, mpg01)
```

Explore the data graphically to discern possible relationships with `mpg01`:
```{r}
...
`cylinders` is negatively associated with `mpg`, new cars are associated with higher `mpg`, and Japanese cars are most fuel efficient, follwed by European, then American. 
...
Fit an LDA model using predictors that seem relevant from visualisations:
```{r}
...
A function that raises a number to the third power:
```{r}
Power <- function(x) {
  print(x^3)
}
...
Power2(3, 8)
```
Some calculations with `Power2`:
```{r}
Power2(10, 3)
Power2(8, 17)
Power2(131, 3)
```
Now a function that `return`s a value:
```{r}
Power3 <- function(x, a) {
  result <- x^a
  return(result)
...
First construct the response variable from `medv`:
```{r}
...
This model gives a 90.8% test accruacy. Try a subset of data based on those that are statistically significant:
...
The more complex model performs better. Now try LDA:
...
```
Of the values for $k$, 5 performed best, with a similar test accuracy to the first logistic regression classifier. 
