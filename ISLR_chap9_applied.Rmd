...
x2[y == 1] <- x2[y == 1] + 2 # Separate classes by adding a constant to one class
plot(x1, x2, col = (3 - y))
...
poly_svm <- svm(y ~ ., data = train, kernel = "radial", degree = 3, scale = FALSE)
...
We can see from the plots and the training error rates that the radial kernel SVM outperforms the support vector classifier on the training data. However, on test data, the linear boundary appears to give better predictions than the SVM with the radial kernel. This suggests that the SVM may be prone to overfitting, even when the true underlying data has a non-linear boundary.
